...
export default function LibraryPage() {
  ...
  const changeTier = async (song: Song, tier: ReferenceTier | null) => {
    try {
      if (!token) return;
      const updated = await updateSongReferenceTier(song.id, tier, token);
  ...
  return <div className="flex-1 overflow-y-auto bg-[#fafcfa]"><div className="mx-auto max-w-6xl px-6 py-8 lg:px-10">
    ...
    <header className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-700">Reference Songs</p><h1 className="mt-1 text-2xl font-bold text-[#333b35]">参考楽曲</h1><p className="mt-2 text-[12px] text-[#7c857e]">歌詞・分類・研究分析の有無を確認し、分析用プロンプトやJSON取込へ進む入口です。</p></div><Link href="/editor" className="rounded-md bg-emerald-700 px-4 py-2 text-[11px] font-bold text-white hover:bg-emerald-800">楽曲を登録</Link></header>
    ...
    {loading ? <p className="py-20 text-center text-[12px] text-[#8b948d]">ライブラリを読み込み中...</p> : visibleSongs.length > 0 ? <div className="overflow-hidden rounded-lg border border-[#e1e7e1] bg-white">{visibleSongs.map((song) => {
      const record = token ? analyses[song.id] : undefined;
      ...
      const counts = analysis ? (analysis.schema_version === "0.3" ? { techniques: analysis.techniques.length, constructions: analysis.constructions.length, endings: analysis.sentence_endings.length, phrases: analysis.phrases.length } : { techniques: analysis.techniques.length, constructions: analysis.expression_patterns.connections.length + analysis.expression_patterns.modifiers.length, endings: analysis.expression_patterns.sentence_endings.length, phrases: analysis.expression_patterns.notable_phrases.length }) : null;
      return <article key={song.id} className="grid gap-3 border-b border-[#edf0ed] px-4 py-4 last:border-0 hover:bg-[#fbfdfb] md:grid-cols-[minmax(180px,1.1fr)_120px_1.8fr_auto] md:items-center">
        ...
        <select disabled={!token} value={song.reference_tier ?? ""} onChange={(event) => changeTier(song, (event.target.value || null) as ReferenceTier | null)} className="rounded-md border border-[#dfe5df] bg-white px-2 py-1.5 text-[10px] text-[#657068] outline-none disabled:opacity-60"><option value="">未分類</option>{Object.entries(TIER_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
        <Link href={`/songs/${song.id}`} className="min-w-0">{analysis && counts ? <><p className="text-[10px] font-bold text-emerald-700">研究分析 v{record?.schema_version} · active</p><p className="mt-1 text-[10px] text-[#69726b]">技法 {counts.techniques} / 構文 {counts.constructions} / 文末 {counts.endings} / フレーズ {counts.phrases}</p>{categories.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{categories.map((category) => <span key={category} className="rounded bg-emerald-50 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700">{category}</span>)}</div>}</> : <span className="inline-flex items-center gap-1.5 text-[10px] text-[#a0a7a1]"><BookOpen size={12} />研究分析なし</span>}</Link>
